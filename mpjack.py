...
Ref:
   ...
   https://docs.google.com/spreadsheets/d/13LDmVFLUzRkc7fkruQ7wdIG78Asg6EemQfnZELk1ZTA/edit?usp=drive_link
...
def get_top(numtop, toplist):
  """
   ...
   """
  logger = logging.getLogger(__name__)
  assert len(numtop) != 0, "numtop is empty."
  #dbp(numtop)
...
def is_new_number_string(url):
  ...
  s = requests.session()
  cookie_obj = requests.cookies.create_cookie(domain="example.com",
                                              name="COOKIE_NAME",
...
def get_random_sample(p55, input_list):
  ...
  while len(r) < JACKPOT_SIX:
    ...
    if num not in r:
      r.append(num)
      ilist.remove(num)
    # else Still keep the original list

  #dbp(r)
  if p55 is True:
    lucky_number = False
    while lucky_number is False:
      num = random.choice(ilist)
      if num not in r:
        r.append(num)
        ilist.remove(num)
        lucky_number = True
      if len(r) > JACKPOT_SIX + 1:
        lucky_number = True
  del ilist
  #dbp(r)
  return r
...
def show_random_6line_list(power55, fn, fticket):
  ...
  if len(rs) == JACKPOT_SIX:
    ...
    r = quicksort(rs[:6])
    #print("r=", r)
    #Adding one more number for power55 jackpot 2
    r.append(rs[6])
...
#cp power968-1018-result.txt power1033-1069-result.txt
#sed -i "s,power968-1018-result,power1033-1069-result,"   ./mpjack.py
# power55 result input file
# presultfile = "./power-result.txt"
  presultfile = "./power1033-1069-result.txt"
  ...
  t, savenewtop, savenewpot = get_potop(False, numcount, savenumpot1,
                                        ticketFile)
  numcount = t
  numcount = savenewtop
  #dbp(numcount)
  ...
  t, psavenewtop, psavenewpot = get_potop(True, pnumcount, pnumcount,
                                          pticketFile)
  # pnumcount = t
  ...
  t, savenewtop, savenewpot = get_potop(False, numcount, savenumpot2,
                                        ticketFile)
  # numcount = t
  numcount = savenewtop
  #dbp(numcount)
  ...
  t, psavenewtop, psavenewpot = get_potop(True, pnumcount, psavenumpot2,
                                          pticketFile)
  # pnumcount = t
  pnumcount = psavenewtop
  dprint(pnumcount)
  ...
  t, savenewtop, savenewpot = get_potop(False, numcount, savenumpot3,
                                        ticketFile)
  # numcount = t
  numcount = savenewtop
  ...
  t, psavenewtop, psavenewpot = get_potop(True, pnumcount, psavenumpot3,
                                          pticketFile)
  # pnumcount = t
  pnumcount = psavenewtop
  dprint(pnumcount)
  ...
  t, savenewtop, savenewpot = get_potop(False, numcount, savenumpot4,
                                        ticketFile)
  numcount = savenewtop
  #dbp(numcount)
  #dbp(savenewpot)
  ...
  t, psavenewtop, psavenewpot = get_potop(True, pnumcount, psavenumpot4,
                                          pticketFile)
  pnumcount = psavenewtop
  dprint(pnumcount)
  ...
  show_top_pot(False, top1list, top2list, top3list, top4list, pot1list,
               pot2list, pot3list, pot4list)
  ...
  show_top_pot(True, ptop1list, ptop2list, ptop3list, ptop4list, ppot1list,
               ppot2list, ppot3list, ppot4list)
  ...
  write_top_pot_to_ticket(pticketFile, ptop1list, ptop2list, ptop3list,
                          ptop4list, ppot1list, ppot2list, ppot3list,
                          ppot4list, pTopPotList)
